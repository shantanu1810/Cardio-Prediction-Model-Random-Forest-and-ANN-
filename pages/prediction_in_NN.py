...
placeholder.success("Loading...")
from tensorflow.keras.models import load_model
import joblib
import pandas as pd
from PIL import Image

Page_title="ANN Based Model"
...
def Neural_Network_Model(age,gender,height,weight,aphi,aplo,cholesterol,gluc,smoke,alco,active,):
...
with st.form('Details'):
    ...
    gluc=st.selectbox(
        'Sugar',['Normal','Above Normal','High']
    )
    smoke=st.selectbox(
        'Do Smoking',
        ['NO','YES']
...
if(submitted):
    ...
    gen={'Male':2,'Female':1}
    preval=Neural_Network_Model(float(age)*365.24,gen[gender],float(height),float(weight),float(aphi),float(aplo),ch_gl[cholesterol],ch_gl[gluc],y_n[smoke],y_n[alco],y_n[active])
    if(preval==1):
        g,al=1,1
        ans="Hello, "+name+"\n\n"
        if(smoke=='YES'):
